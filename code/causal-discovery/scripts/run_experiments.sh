...
do
  ...
  docker exec -it cd-manager /bin/bash -c "source activate cd-env && python /causal-discovery/cdrl/setup_experiments.py --experiment_part ${expPart} --which ${expWhich} --experiment_id ${expId} --instance_name ${instanceName} --budget ${expBudget}"
