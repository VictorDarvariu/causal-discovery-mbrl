...
class RandomShootingAgent(Agent):
    ...
    def create_sim_policy(self):
        ...
        self.sim_policy_inst = sim_policy_class(self.local_random, enforce_acyclic=self.environment.enforce_acyclic, max_parents=self.environment.max_parents)
    ...
    def eval(self, g_list, phase):
        ...
        for j in range(sim_budget):
            ...
            while True:
                ...
                rem_budget = self.environment.apply_action_in_place(state, phase, chosen_action, rem_budget, self.environment.enforce_acyclic, self.environment.max_parents)
                current_depth += 1
