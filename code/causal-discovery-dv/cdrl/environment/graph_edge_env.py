...
class DirectedGraphEdgeEnv(object):
    ...
    """
    def __init__(self, disc_instance, reward_function, initial_edge_budgets, enforce_acyclic, max_parents, **kwargs):
    ...
    def setup(self, g_list, phase):
        ...
        self.used_edge_budgets = np.zeros(len(g_list), dtype=np.float64)
        self.exhausted_budgets = np.zeros(len(g_list), dtype=bool)

        for i in range(len(self.g_list)):
            g = g_list[i]
            g.first_node = None
            g.populate_banned_actions(self.phase, budget=self.edge_budgets[i], enforce_acyclic=self.enforce_acyclic, max_parents=self.max_parents)

        self.rewards = np.zeros(len(g_list), dtype=np.float64)
    ...
    def get_graph_edge_choices_for_idx(self, i):
        ...
        g = self.g_list[i]
        return self.get_graph_edge_choices(g, self.phase, self.enforce_acyclic, self.max_parents)

    @staticmethod
    def get_graph_edge_choices(g, phase, enforce_acyclic, max_parents):
        ...
        if phase == EnvPhase.CONSTRUCT:
            ...
            for first in valid_acts:
                banned_second_nodes = g.get_invalid_edge_ends(phase, first, enforce_acyclic=enforce_acyclic, max_parents=max_parents)
    ...
    @staticmethod
    def apply_action(g, phase, action, remaining_budget, enforce_acyclic, max_parents, copy_state=False):
        """Applies the chosen action by modifying the graph structure of the state."""
        if g.first_node is None:
            # Edge stub has not been picked
            if copy_state:
                g_ref = g.copy()
            else:
                g_ref = g
            g_ref.first_node = action
            g_ref.populate_banned_actions(phase, budget=remaining_budget, enforce_acyclic=enforce_acyclic, max_parents=max_parents)
            # selection doesn't cost anything.
            return g_ref, remaining_budget
        else:
            # Edge stub has been picked
            ...
            new_g.populate_banned_actions(phase, budget=updated_budget, enforce_acyclic=enforce_acyclic, max_parents=max_parents)
            return new_g, updated_budget

    @staticmethod
    def apply_action_in_place(g, phase, action, remaining_budget, enforce_acyclic, max_parents):
        ...
        if g.first_node is None:
            g.first_node = action
            g.populate_banned_actions(phase, budget=remaining_budget, enforce_acyclic=enforce_acyclic, max_parents=max_parents)
            ...
            updated_budget = remaining_budget - edge_cost
            g.populate_banned_actions(phase, budget=updated_budget, enforce_acyclic=enforce_acyclic, max_parents=max_parents)
    ...
    def step(self, actions):
        ...
        for i in range(len(self.g_list)):
            if not self.exhausted_budgets[i]:
                ...
                self.g_list[i], updated_budget = self.apply_action(self.g_list[i], self.phase, actions[i], remaining_budget, self.enforce_acyclic, self.max_parents)
