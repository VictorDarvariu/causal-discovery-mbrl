...
class GreedyAgent(BaselineAgent):
    ...
    def pick_actions_using_strategy(self, t, i):
        ...
        initial_value = self.get_initial_value(i, g)
        best_val = float("-inf")
        ...
        for first, second in edge_choices:
            ...
            edge_val = self.get_edge_value(initial_value, g, next_g, first, second)
            self.obj_fun_eval_count += 1

            if edge_val > best_val:
                if self.is_deterministic or ((not self.is_deterministic) and self.local_random.random() >= 0.5):
                    best_val = edge_val
    ...
    def get_initial_value(self, i, g):
        return self.environment.rewards[i]

    def get_edge_value(self, initial_value, g, next_g, first, second):
