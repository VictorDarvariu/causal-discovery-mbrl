...
class DAGState(object):
    """
    This class implements the state for the decision-making process defined over DAGs. Actions decided by the agents modify this state.
    The state contains information about the graph topology (e.g., the intermediate graph structure as decided so far)
    and complex bookkeeping information that is required to determine which next actions are valid choices.

    It also implements the incremental algorithm for determining cycle-inducing candidate edges.
    """
    def __init__(self, g, init_tracking=True):
        ...
        # "dynamic edges" are not added to the topology straight away but stored; used extensively in the MCTS simulation policy.
    ...
    def populate_banned_actions(self, phase, budget=None, enforce_acyclic=True, max_parents=None):
        ...
        if self.first_node is None:
            self.banned_actions = self.get_invalid_first_nodes(phase, enforce_acyclic=enforce_acyclic, max_parents=max_parents)
        else:
            self.banned_actions = self.get_invalid_edge_ends(phase, self.first_node, enforce_acyclic=enforce_acyclic, max_parents=max_parents)

    def get_invalid_first_nodes(self, phase, enforce_acyclic=True, max_parents=None):
    ...
    def get_invalid_edge_ends(self, phase, query_node, enforce_acyclic=True, max_parents=None):
        ...
        if phase == EnvPhase.CONSTRUCT:
            ...
            if max_parents is not None:
                results.update(
                    {n for n in self.all_nodes_set if self.node_in_degrees[n] == max_parents}
                )
            ...
            if self.dynamic_edges is not None:
                already_removed = {entry[1] for entry in self.dynamic_edges if entry[0] == query_node}
    ...
    def to_pgmpy(self, node_aliases):
        """Convert to pgmpy DAG object."""
        pgm_G = DAG()
        pgm_G.add_nodes_from(node_aliases)
        aliased_edges = self.get_edges_with_aliases(node_aliases)
        pgm_G.add_edges_from(aliased_edges)
        return pgm_G
