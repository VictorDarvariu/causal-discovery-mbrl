...
"""
This script and associated source files were integrated from the RL-BIC repository.
The only modification that was made is to this file, so as to compute and report the value of the reward function (see `compute_gt_rewards`),
which the original code was lacking.
...
try:
    ...
    print("could not import CAM -- CAM pruning will not work.")
    pass
...
def main():
    ...
    config.scores_dir = '{}/scores'.format(output_dir)
    config.final_graphs_dir = '{}/final_graphs'.format(output_dir)
    ...
    create_dir(config.scores_dir)
    create_dir(config.final_graphs_dir)
    ...
    sess_config = tf.ConfigProto(log_device_placement=False, intra_op_parallelism_threads=1, inter_op_parallelism_threads=1)
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for i in (range(1, config.nb_epoch + 1)):
            ...
            the_scores, graphs_feed = sess.run([actor.decoder.mask_scores, actor.graphs], feed_dict={actor.input_: input_batch})
            ...
            # model selection for simulation policy scores
            ...
            if (i+1) % lambda_iter_num == 0:
                ...
                np.save('{}/DAG_orig.npy'.format(config.final_graphs_dir), np.array(graph_batch.T))
                np.save('{}/DAG_pruned.npy'.format(config.final_graphs_dir), np.array(graph_batch_pruned.T))
                ...
                gt_reward, gt_pruned_reward = compute_gt_rewards(training_set.inputdata, reg_type, score_type, graph_batch, graph_batch_pruned)
                _logger.info('GT reward BEFORE pruning: {}'.format(gt_reward))
                _logger.info('GT reward AFTER pruning: {}'.format(gt_pruned_reward))
...
def compute_gt_rewards(input_data, reg_type, score_type, graph_batch, graph_batch_pruned):
    rfun = ContinuousVarsBICRewardFunction(input_data, reg_type=reg_type, score_type=score_type)
    reward = rfun.calculate_reward_single_graph(graph_batch)
    reward_pruned = rfun.calculate_reward_single_graph(graph_batch_pruned)
    return reward, reward_pruned
